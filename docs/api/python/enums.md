...
In **CRCGCB**, **CBCGCR**, and their **_FLASHED** variants, **screenBuffer** and **automapBuffer** store all red 8-bit values then all green values and then all blue values, each channel is considered separately. As matrices (`np.ndarray`) they have `(3, y, x)` shape.

In **RGB24**, **BGR24**, and their **_FLASHED** variants, **screenBuffer** and **automapBuffer** store 24 bit RGB triples. As matrices (`np.ndarray`) they have `(y, x, 3)` shape.

In **RGBA32**, **ARGB32**, **BGRA32**, **ABGR32**, and their **_FLASHED** variants, **screenBuffer** and **automapBuffer** store 32 bit sets of RBG + alpha values. As matrices (`np.ndarray`) they have `(y, x, 4)` shape.

In **GRAY8**, **GRAY8_FLASHED**, and **DOOM_256_COLORS8** format **screenBuffer** and **automapBuffer** store single 8 bit values. As matrices (`np.ndarray`) they have `(y, x)` shape.
