...
void Vizia_GameVarsTic(){
    ...
    viziaGameVars->NET_GAME = netgame || multiplayer;
    ...
    if(viziaPlayer->mo) {
        ...
        viziaGameVars->PLAYER_DEAD = viziaPlayer->playerstate == PST_DEAD || viziaPlayer->mo->health <= 0;
    }
    else {
        viziaGameVars->PLAYER_HAS_ACTOR = false;
