...
class LSTMExtractor(nn.Module):
    ...
    def __init__(
        ...
        #shared_net = nn.LSTM(feature_dim, 128, 1)
    ...
    #def forward(self, features: th.Tensor, rnn_state ) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
    def forward(self, features: th.Tensor ) -> Tuple[th.Tensor, th.Tensor]:
        """
        :return: latent_policy, latent_value of the specified network.
            If all layers are shared, then ``latent_policy == latent_value``
        """
        #shared_latent, rnn_state = self.shared_net(features)
        #return self.policy_net(shared_latent), self.value_net(shared_latent), rnn_state
        shared_latent = self.shared_net(features)
        return self.policy_net(shared_latent), self.value_net(shared_latent)
