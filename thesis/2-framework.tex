...
VIZIA Environment was build around FPS (First Person Shooter) game Doom and its game engine's modernised version -- ZDoom. 
%troche niezgrabne to zdanie
It is shown in Figure~\ref{fig:doom}. 
%it czyli?
Doom was chosen out of 6 other recognisable FPS games considered: 
...
Lack of scripting support and inaccessible screen buffer were considered as a critical factors for rejecting such game %o jakie such game tutaj chodiz? 
and accessing engine's logic only by SDK was considered as a major drawback, so Unreal Tournament 2004 and Half-Life 2 could not be accepted.
Multi-threaded or client-server architecture would make it difficult to fully control speed of the execution, therefore Quake III Arena had to be rejected.%niezgrabne zdanie. do poprawki
Doom 3 had to be ignored not only for this reason, but also because of its complexity, Windows-only tools and OS-dependent rendering mechanisms.
\begin{table}[]
\centering
...
Despite active community and great capabilities Unreal Tournament was out of question, because of its very high system requirements.




ZDoom based Doom met most of the given requirements and allowed to implement features that would be barely achievable in other games e.g., off-screen rendering and reward computation.
Although Doom was not highly popular in research on reinforcement learning, it's recognizability exceeds Unreal Tournament brand.
%nie ma ciagu logicznego
Originally Doom was designed to work in 320x240 resolution and even so
%even so? despite the fact that...?
modern implementations allow bigger resolutions, it still utilises same low resolution textures and a rendering algorithm which makes it perfect for generating small visual input data (320x240 or smaller) for reinforcement learning algorithm.
%za duzo odwolan do RL. Szerzej: AI
Code analysis showed that access to game logic should be easy, as it is not object-oriented code.
%it, czyli?
%znow brak logicznego ciagu
...
Doom's gameplay is quite simple compared to newer FPS games as it
%czy to aby na pewno prawda? a jesli tak to jakie ma to w tym miejscu naczenie?
 consists mostly of running around and shooting and possible actions could be easily translated for reinforcement learning needs. 
...
%WEŹ COŚ SENSOWNEGO TU WYMYŚL
Linux has been chosen as a target operating system for this environment.
...
It allowed to use Boost::interprocess library in both ZDoom and game controller, as ZDoom was written in C++, what made connecting game controller and game easier than it would be using low level language on the engine side and high level language for the controller.%zdanie zbyt dlugie. podzielic na 2-3
...
It was also developed in C++, which allowed to provide bindings to Python and Java, with possibility to add another languages e.g., Lua, Julia.
%it=API?
...
Creation of test environments has been made possible by using tools developed by Doom community for maps and scripts editing: SLADE 3[] and Doom Bulider 2[]. Those tools utilizes ACC --- a compilter for Action Code Script (ACS), language, which is supported in ZDoom engine.
...
Depth buffer could be find usefull for learning depth perception
%dodatkowo: jest to informacja, ktora moze posiadac robot w podobnym srodowisku
 by agent so it was added to the engine as presented in Figure~\ref{fig:zbuffer}.
