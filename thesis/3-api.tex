...
\end{clinee}
    Process a specified number of tics. If stateUpdate is set the state will be updated after last processed tic and a new reward will be calucated. To get new state use \emph{getState} and to get the new reward use  \emph{getLastReward}. If stateUpdate is not set but renderOnly is set the state will not be updated but a new frame will be rendered after last processed tic. To get the new frame use \emph{getGameScreen}.

\vspace{20pt}
\begin{clinee}
	void DoomGame::advanceAction(unsigned int tics);
\end{clinee}
    Process a specified number of tics, updates state and calulates a new reward. Short for \emph{advanceAction(tics, true, false)}.

\vspace{20pt}
\begin{clinee}
	void DoomGame::advanceAction();
\end{clinee}
	Process one tic, updates the state and calulates a new reward. Short for \emph{advanceAction(1, true, false)}.
...
Function combining usibility of \emph{setAction}, \emph{advanceAction} and \emph{getLastReward}. Sets the player's action for the next tics, process a specified number of tics, updates the state and calulates a new reward, which is returned.
...
Function combining usibility of \emph{setAction}, \emph{advanceAction} and \emph{getLastReward}. Sets the player's action for the next tics, process one tic, updates the state and calulates a new reward, which is returned. Short for \emph{makeAction(action, 1}.
...
\end{clinee}
	Returns a pointer the screen buffer.
...
\end{clinee}
	Sends the command to Doom console. Can be used for cheats, multiplayer etc. For more details consult Zdoom Wiki Webpage\cite{zdoom-wiki}.
...
\begin{itemize}
 ...
 \item RGB24 - channel of RGB values stored in 24 bits, where R value is stored in oldes 8 bits
 \item RGBA32 - channel of RGBA values stored in 32 bits, where R value is stored in oldes 8 bits
 \item ARGB32 - channel of ARGB values stored in 32 bits, where A value is stored in oldes 8 bits
 \item CBCGCR - 3 channels of 8 bit values in BGR order
 \item CBCGCRDB - 4 channels of 8 bit values in BGR + depth buffer order
 \item BGR24 - channel of BGR values stored in 24 bits, where B value is stored in oldes 8 bits
 \item BGRA32 - channel of BGRA values stored in 32 bits, where B value is stored in oldes 8 bits 
 \item ABGR32 - channel of ABGR values stored in 32 bits, where A value is stored in oldes 8 bits
